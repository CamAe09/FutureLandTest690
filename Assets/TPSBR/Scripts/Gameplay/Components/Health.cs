	...
	public sealed class Health : ContextBehaviour, IHitTarget, IHitInstigator
	{
		...
		void IHitTarget.ProcessHit(ref HitData hitData)
		{
			...
			if (IsAlive == false)
			{
				...
				// Quest tracking for eliminations
				if (hitData.InstigatorRef == Runner.LocalPlayer && hitData.InstigatorRef.IsRealPlayer)
				{
					bool isHeadshot = hitData.HitType.ToString().Contains("Head");
					float distance = Vector3.Distance(hitData.Position, _agent.transform.position);
					
					if (TPSBR.BattleRoyaleQuestTracker.Instance != null)
					{
						string weaponType = "Unknown";
						TPSBR.BattleRoyaleQuestTracker.Instance.OnEliminationScored(weaponType, isHeadshot, distance);
					}
				}
			}
		}
		...
		void IHitInstigator.HitPerformed(HitData hitData)
		{
			if (hitData.Amount > 0 && hitData.Target != (IHitTarget)this && Runner.IsResimulation == false)
			{
				...
				// Quest tracking for damage dealt
				if (hitData.InstigatorRef == Runner.LocalPlayer && hitData.Action == EHitAction.Damage)
				{
					if (TPSBR.BattleRoyaleQuestTracker.Instance != null)
					{
						TPSBR.BattleRoyaleQuestTracker.Instance.OnDamageDealt(hitData.Amount);
					}
				}
			}
		}
		...
		private void ApplyHit(ref HitData hit)
		{
			...
			if (hit.Action == EHitAction.Damage)
			{
				...
				// Quest tracking for storm damage survived
				if (hit.HitType == EHitType.ShrinkingArea && hit.InstigatorRef == Runner.LocalPlayer && IsAlive)
				{
					if (TPSBR.BattleRoyaleQuestTracker.Instance != null)
					{
						TPSBR.BattleRoyaleQuestTracker.Instance.OnStormDamageTaken(hit.Amount);
					}
				}
			}
